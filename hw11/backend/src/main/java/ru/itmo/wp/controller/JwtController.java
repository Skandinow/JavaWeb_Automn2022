...
public class JwtController {
    ...
    @PostMapping("jwt")
    public String create(@RequestBody @Valid UserEnterCredentials userEnterCredentials, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            throw new ValidationException(bindingResult);
        }

        User user = userService.findByLoginAndPassword(userEnterCredentials.getLogin(), userEnterCredentials.getPassword());
