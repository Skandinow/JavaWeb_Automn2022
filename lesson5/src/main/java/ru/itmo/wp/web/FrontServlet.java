...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        Method method = null;
        Class<?>[] allowedType = new Class[]{HttpServletRequest.class,
                Map.class};
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            Method[] classMethods = clazz.getDeclaredMethods();
            Arrays.sort(classMethods, Comparator.comparing(Method::hashCode));
            for (Method classMethod : classMethods) {
                if (!classMethod.getName().equals(route.getAction())) {
                    continue;
                }
                if (Arrays.stream(classMethod.getParameterTypes()).allMatch(Arrays.asList(allowedType)::contains)) {
                    method = classMethod;
                    break;
                }
            }
        ...
        //thats mine
        //проблема номер1 могу вызвать не тот элеменt
        List<Object> arr = new ArrayList<>();
        for (Class<?> parameter : method.getParameterTypes()) {
            if (parameter.equals(HttpServletRequest.class)) {
                arr.add(request);
            } else if (parameter.equals(Map.class)) {
                arr.add(view);
            } else {
                arr.add(null);
            }
        ...
        try {
            method.invoke(page, arr.toArray());
        ...
        String langName = simpleName;
        if (lang != null && !lang.equals("en")) {
            langName += "_" + lang;
        }
//        System.out.println(simpleName + "!");
//        System.out.println(langName + "?");
//        System.out.println(pageClass.getSimpleName());


        try {
            template = newTemplate(langName + ".ftlh");
